...
--JSON = (loadfile "C:\\Program Files (x86)\\clink\\0.4.9\\JSON.lua")()
-- -*- coding: utf-8 -*-
--
...
local function npm_scripts_match_generator()
	local fileContent = read_file("package.json")
	if fileContent == nil then return nil end
	local lua_value = JSON:decode(fileContent)
	local keyset={}
	local n=0
	for k,v in pairs(lua_value["scripts"]) do
	  n=n+1
	  keyset[n]=k
	end
    return keyset;
...
-- REF: https://docs.npmjs.com/cli/start
local npm_argument_tree = {
    "access", "adduser", "audit",
    "bin", "bugs", "build", "bundle",
    "cache", "ci", "completion", "config",
    "dedupe", "deprecate", "dist-tag", "docs", "doctor",
    "edit", "explore",
	...
	"run-script" .. clink.arg.new_parser({ npm_scripts_match_generator }),
    -- In order to trigger autocomplete when typing "st<TAB>", so I disabled these commands:
    -- "star", "stars", "stop",
	"start", "search", "shrinkwrap",
    "team", "test", "token",
